...
export function testDb(): Database {
  database ??= drizzle(testSql(), { schema });
  return database;
}
...
export async function createTestTenant(
  ...
  await db.insert(schema.tenants).values({
    id: tenantId,
    slug: options.slug ?? `test-${tenantId.slice(0, 8)}`,
    name: 'Comercio de prueba',
    planCode: options.plan ?? 'free',
    baseCurrency: 'USD',
  ...
  await db.insert(schema.memberships).values({ tenantId, userId, role, status: 'active' });
...
export async function dropTestTenant(tenantId: TenantId): Promise<void> {
  await testDb().execute(sql`delete from public.tenants where id = ${tenantId}`);
}
